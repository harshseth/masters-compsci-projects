...
int main(int argc, char **argv){
	...
	static char sBuffer[MAXBUFFER], fileName[50], sPacketBuffer[PACKETSIZE], ackBuffer[10], sSegmentBuffer[SEGMENTSIZE];
	unsigned short serverPort;
	int fileNameLen, ackLen, lastSentSeqNum = 0;
	...
	while(listenFlag){
		while(!feof(pFile)){
			...
			if(tempSize < PACKETSIZE){
				...
				rudphdr->FIN = 1;
				rudphdr->finalOffset = (unsigned short int)tempSize;
				rudphdr->ackNumber = 0;
				...
				if(sendto(serverSockDesc,sSegmentBuffer,SEGMENTSIZE,0, (struct sockaddr *)&clientSockAddr, sizeof(clientSockAddr))!= SEGMENTSIZE)
					fprintf(stderr,"\nsendto() sent a different number of bytes than originally expected! ");
				printf("\nPacket%d sent %s!",rudphdr->seqNumber, sPacketBuffer);
			}
		...
		if(rudphdr->ackNumber == lastSentSeqNum)
			listenFlag = 0;
