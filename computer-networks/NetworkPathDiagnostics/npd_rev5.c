...
static FILE *fp1, *fp2, *fp3, *fp4, *fp5, *filePointer, *fp, *fpx;
...
static char *string11, *string22 , *string33, *domainName, *string2_t ,*string2_t1 ;
static char domain1[100], domain2[100];


int TLD(const char *string2, char domain[100])
{

char line[300] = {}, linex[300] = {};
c=0;
char prepend[100] = {}, prepend_t[150] = {},rest[100] = {};
int length = 0,j=0,t=1,x = 0,x1 = 0, m=0,check=0,z=0,n=0,restl = 0,linelen = 0,longest_found=0,lno=1;
strcpy(string2_t,string2);
int lens = 0,is = 0,zs = 0,checks=0;
int checkx=0,zx = 0,ix = 0,finally=0,xx = 0;
...
int TLDINIT(const char *string1i, char *string2i, char *string3i)
{

	int d1_length = 0, d2_length = 0,imt=0,checkmt=0;
	if(string2i==NULL)
	{
...
int main(int argc, char **argv ){
	...
	char *input = (char *)calloc(1,50*sizeof(char)), *a = (char *)calloc(1,sizeof(char)), *inputString = (char *)calloc(1,50*sizeof(char)), *isTok = (char *)calloc(1,50*sizeof(char)),*lhTok = (char *)calloc(1,50*sizeof(char));
	...
	concatString = (char *)calloc(1,100*sizeof(char));
	lastHopAddr = (char *)calloc(1,100*sizeof(char));
	...
	lastHopAddrs = (char **)calloc(100,sizeof(char *));
	lastHopHostNames = (char **)calloc(100,sizeof(char *));
	lastHopAddrs24 = (char **)calloc(100,sizeof(char *));
	fileInput = (char *)calloc(1,50*sizeof(char));
	prohibited = (char *)calloc(1,5*sizeof(char));
	lastHopBGPPrefixes = (char **)calloc(100,sizeof(char *));
	lastHopBGPPrefixes24 = (char **)calloc(100,sizeof(char *));
	lastHopDomains = (char **)calloc(100,sizeof(char *));
	lastHop24 = (char *)calloc(1,100*sizeof(char));
	dest24 = (char *)calloc(1,100*sizeof(char));
	...
	string2_t = (char *)calloc(1,100*sizeof(char));
	string2_t1 = (char *)calloc(1,100*sizeof(char));
	
	strcpy(concatString,"traceroute ");
	
	for(i = 0; i<100 ; i++) 
		if( (lastHopAddrs[i] = (char *)calloc(1,20*sizeof(char))) == NULL )
			  fprintf(stderr, "\nMemory cannot be allocated to lastHopHostAddrs");
	for(i = 0; i<100 ; i++) 
		if( (lastHopAddrs24[i] = (char *)calloc(1,32*sizeof(char))) == NULL )
			  fprintf(stderr, "\nMemory cannot be allocated to lastHopHostAddrs24");
	for(i = 0; i<100 ; i++) 
		if( (lastHopHostNames[i] = (char *)calloc(1,100*sizeof(char))) == NULL )
			  fprintf(stderr, "\nMemory cannot be allocated to lastHopHostNames");	
	for(i = 0; i<100 ; i++) 
		if( (lastHopBGPPrefixes[i] = (char *)calloc(1,100*sizeof(char))) == NULL )
			  fprintf(stderr, "\nMemory cannot be allocated to lastHopBGPPrefixes");
	for(i = 0; i<100 ; i++) 
		if( (lastHopBGPPrefixes24[i] = (char *)calloc(1,100*sizeof(char))) == NULL )
